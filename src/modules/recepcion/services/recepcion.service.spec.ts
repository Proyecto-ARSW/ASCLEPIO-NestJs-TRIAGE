...
describe('RecepcionService', () => {
  ...
  const mockCoreNotifier = { notificarTurnoCreado: jest.fn().mockResolvedValue(undefined) };
  const mockTriageGateway = {
    emitToDashboardEnfermeros: jest.fn(),
    emitAlertaCritica: jest.fn(),
  };
  const mockEventPublisher = { publishTurnoCreado: jest.fn().mockResolvedValue(undefined) };
  ...
  const mockTurno = { id: 'turno-uuid-1', numero_turno: 1, hospital_id: 1, paciente_id: dtoBase.paciente_id };
  const mockRegistro = { id: 'registro-uuid-1' };
  const mockClasificacion = {
    nivel_sugerido: 3,
    confianza: 0.85,
    comentarios: 'Normal',
    probabilidades: { nivel_1: 0.01, nivel_2: 0.05, nivel_3: 0.85, nivel_4: 0.07, nivel_5: 0.02 },
  ...
  describe('procesarIngreso', () => {
    ...
    it('debería generar numero_turno = ultimo + 1 si hay turnos previos', async () => {
    ...
    it('debería detectar taquicardia (FC > 100) como alerta vital', async () => {
      const dto = { ...dtoBase, frecuencia_cardiaca: 110 };
      mockPrisma.turnos.findFirst.mockResolvedValue(null);
      mockPrisma.turnos.create.mockResolvedValue(mockTurno);
      mockPrisma.registros_triage.create.mockResolvedValue(mockRegistro);
      mockPrisma.turnos.update.mockResolvedValue({});
      ...
      const result = await service.procesarIngreso(dto);
    ...
    it('debería detectar hipoxemia (SpO2 < 92%) como alerta vital', async () => {
      const dto = { ...dtoBase, saturacion_oxigeno: 88 };
      mockPrisma.turnos.findFirst.mockResolvedValue(null);
      mockPrisma.turnos.create.mockResolvedValue(mockTurno);
      mockPrisma.registros_triage.create.mockResolvedValue(mockRegistro);
      mockPrisma.turnos.update.mockResolvedValue({});
    ...
    it('debería detectar hipotensión (PAM < 65) como alerta vital', async () => {
      // PAM = (90 + 2*50)/3 = 63.3
      const dto = { ...dtoBase, presion_sistolica: 90, presion_diastolica: 50 };
      mockPrisma.turnos.findFirst.mockResolvedValue(null);
      mockPrisma.turnos.create.mockResolvedValue(mockTurno);
      mockPrisma.registros_triage.create.mockResolvedValue(mockRegistro);
      mockPrisma.turnos.update.mockResolvedValue({});
    ...
    it('debería detectar fiebre alta (T > 39°C) como alerta vital', async () => {
      const dto = { ...dtoBase, temperatura: 39.5 };
      mockPrisma.turnos.findFirst.mockResolvedValue(null);
      mockPrisma.turnos.create.mockResolvedValue(mockTurno);
      mockPrisma.registros_triage.create.mockResolvedValue(mockRegistro);
      mockPrisma.turnos.update.mockResolvedValue({});
    ...
    it('debería crear alerta crítica y emitir WebSocket cuando hay alertas vitales', async () => {
      const dto = { ...dtoBase, saturacion_oxigeno: 85 };
      const mockAlerta = { id: 'alerta-critica-1' };
      mockPrisma.turnos.findFirst.mockResolvedValue(null);
      mockPrisma.turnos.create.mockResolvedValue(mockTurno);
      mockPrisma.registros_triage.create.mockResolvedValue(mockRegistro);
      mockPrisma.turnos.update.mockResolvedValue({});
      ...
      await service.procesarIngreso(dto);

      expect(mockPrisma.alertas_criticas.create).toHaveBeenCalledWith(
        expect.objectContaining({ data: expect.objectContaining({ turno_id: mockTurno.id }) }),
      );
      expect(mockTriageGateway.emitAlertaCritica).toHaveBeenCalled();
    });

    it('debería notificar WebSocket de dashboard enfermeros', async () => {
      mockPrisma.turnos.findFirst.mockResolvedValue(null);
      mockPrisma.turnos.create.mockResolvedValue(mockTurno);
      mockPrisma.registros_triage.create.mockResolvedValue(mockRegistro);
      mockPrisma.turnos.update.mockResolvedValue({});
    ...
    it('debería detectar taquipnea (FR > 24) y shock index elevado', async () => {
      // FR > 24, SI = 110/80 = 1.375 > 1.0
      const dto = { ...dtoBase, frecuencia_respiratoria: 26, frecuencia_cardiaca: 110, presion_sistolica: 80 };
      mockPrisma.turnos.findFirst.mockResolvedValue(null);
      mockPrisma.turnos.create.mockResolvedValue(mockTurno);
      mockPrisma.registros_triage.create.mockResolvedValue(mockRegistro);
      mockPrisma.turnos.update.mockResolvedValue({});
